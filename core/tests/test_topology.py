...
class TestTopologyValidation:
    def test_file_not_found_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError, match="does not exist"):
            Simulation(filename=tmp_path / "nonexistent.txt", maxtime=10.0, debug=False)

    def test_non_integer_token_raises(self, make_topo):
        topo = make_topo("2\nabc\n")
        with pytest.raises(ValueError, match="must be integers"):
            Simulation(filename=topo, maxtime=10.0, debug=False)
    ...
    def test_valid_topology_loads(self, make_topo):
        topo = make_topo("2\n1\n")
        sim = Simulation(filename=topo, maxtime=10.0, debug=False)
